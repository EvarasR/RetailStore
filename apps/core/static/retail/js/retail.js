(function(){
  ...
  async function getJSON(url){
    ...
    if(!r.ok || data.ok === false) throw new Error(data.mensaje || 'Error de servidor');
    return data;
  }
  async function postForm(url, obj){
    ...
    const data = await r.json().catch(()=>({ok:false,mensaje:'Respuesta inválida'}));
    if(!r.ok || data.ok === false){ const err = new Error(data.mensaje || 'Error de servidor'); err.payload = data; throw err; }
    return data;
  }
  function setupTheme(){
  ...
  function setupAdmin(){ const load={dashboard:adminResumen,productos:adminProductos,pedidos:adminPedidos,inventario:adminInventory,proveedores:adminSuppliers,reportes:adminReports}; adminResumen().catch(e=>toast(e.message)); $$('[data-admin-section]').forEach(btn=>btn.addEventListener('click',()=>{const s=btn.dataset.adminSection; $$('[data-admin-panel]').forEach(p=>p.classList.toggle('rp-hidden',p.dataset.adminPanel!==s)); (load[s]||adminResumen)().catch(e=>toast(e.message));})); $('#admin-product-search')?.addEventListener('input',()=>adminProductos().catch(e=>toast(e.message))); $('#admin-product-state')?.addEventListener('change',()=>adminProductos().catch(e=>toast(e.message))); $('#admin-order-state')?.addEventListener('change',()=>adminPedidos().catch(e=>toast(e.message))); document.addEventListener('click', async e=>{ const pub=e.target.closest('[data-publish]'), pau=e.target.closest('[data-pause]'), dis=e.target.closest('[data-disable]'); try{ if(pub){await postForm(`/panel/api/productos/${num(pub.dataset.publish)}/publicar/`,{});toast('Producto publicado');adminProductos();} if(pau){await postForm(`/panel/api/productos/${num(pau.dataset.pause)}/pausar/`,{});toast('Producto pausado');adminProductos();} if(dis){await postForm(`/panel/api/productos/${num(dis.dataset.disable)}/desactivar/`,{});toast('Producto desactivado');adminProductos();} }catch(err){toast(err.message)} }); }

  document.addEventListener('DOMContentLoaded',()=>{ setupTheme(); const page=document.body.dataset.page; if(page==='cliente') setupClient().catch(e=>toast(e.message)); if(page==='perfil') setupPerfil().catch(e=>toast(e.message)); if(page==='admin') setupAdmin(); });
})();
