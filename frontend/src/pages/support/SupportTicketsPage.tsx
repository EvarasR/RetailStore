      ...
      <div className="ops-table-card">
        <div className="ops-table-header">
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
            <MessageSquare size={18} color="var(--tt-color-primary)" />
            ...
            <tbody>
              {loading ? (
                ...
                filteredTickets.map((t) => (
                  <tr key={t.cod_ticket}>
                    <td style={{ color: 'var(--tt-color-text-light)', fontWeight: 600 }}>#{t.cod_ticket}</td>
                    <td style={{ fontWeight: 600 }}>
                      <div>{t.cliente || `Usuario #${t.cod_usuario || 'N/A'}`}</div>
                      {t.email && (
                        <div style={{ fontSize: '0.75rem', color: 'var(--tt-color-text-light)' }}>{t.email}</div>
                      )}
                    </td>
                    <td>
                      <strong style={{ color: 'var(--tt-color-text-main)', display: 'block', fontSize: '0.9rem' }}>
                        {t.asunto}
                      </strong>
                      <span style={{ fontSize: '0.75rem', color: 'var(--tt-color-primary)' }}>{t.categoria}</span>
                    </td>
                    <td>
                      <span
                        className={
                    ...
                    <td style={{ color: 'var(--tt-color-text-muted)' }}>{t.fecha}</td>
                    <td>
                      <div style={{ display: 'flex', gap: '0.4rem' }}>
                        ...
                        {t.estado !== 'RESUELTO' && t.estado !== 'CERRADO' && (
                          <button
                            ...
                            className="tt-btn tt-btn--secondary"
                            style={{ padding: '0.25rem 0.55rem', fontSize: '0.75rem', display: 'flex', alignItems: 'center', gap: '0.25rem', color: 'var(--tt-color-success)', borderColor: 'var(--tt-color-success)' }}
